...
function listenLinks() {
	...
	async function showPreview(link) {
		...
		tooltip.setAttribute("id", `tooltip-${href}`);
		...
		// think about removing the attribute
		link.setAttribute("aria-describedby", `tooltip-${href}`);
		body.appendChild(tooltip);

		const { top, left } = getTooltipLocation("bottom", link, tooltip);
		tooltip.style.cssText += `
        top: ${top}px;
        left: ${left}px;
        opacity: 1;
...
function getTooltipLocation(position, el, tooltip, view = window) {
